<?php
function gavias_lozin_render_block($key) {
...
function gavias_lozin_makeid($length = 5){
...
function gavias_lozin_base_url(){
  global $base_url;
  $theme_path = drupal_get_path('theme', 'gavias_lozin');
  return $base_url . '/' . $theme_path . '/';
}

function gavias_lozin_preprocess_node(&$variables) {
  ...
  if ($variables['node']->getType() == 'article') {
    ...
    $variables['theme_uri'] = base_path() . drupal_get_path('theme', 'gavias_lozin');
  }
}

function gavias_lozin_preprocess_node__listing(&$variables) {
  $node = $variables['node'];
  $listing_video = '';
  ...
  if(in_array($view_mode, $allowed_view_modes)) {
    ...
    gavias_lozin_add_regions_to_node($allowed_regions, $variables);
  }
}

function gavias_lozin_preprocess_user(&$variables) {
  $allowed_regions = ['listings_user'];
  gavias_lozin_add_regions_to_node($allowed_regions, $variables);
}


function gavias_lozin_preprocess_node__portfolio(&$variables){
  $node = $variables['node'];
...
function gavias_lozin_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function gavias_lozin_preprocess_breadcrumb(&$variables){
...
*/
function gavias_lozin_add_regions_to_node($allowed_regions, &$variables) {
